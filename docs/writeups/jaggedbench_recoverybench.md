...
We tested how often models repeated errors handed to them, both ones we manually created and those organically harvested from reasoning traces. We focused on problems of moderate difficulty; models might rationally defer to given solutions on very hard problems, given that their base rate of solving is very low, and on very easy problems, they can simply ignore the information and write the solution. We find evidence that models often defer to incorrect solutions, 

## Methods and Results

In our initial pass, we tested GPT-OSS-120B and Minimax m2.5 on LiveCodeBench items and found that they deferred more to manually-generated errors than those extracted from actual failed model reasoning traces (9/45 vs 39/42, Fisher p 1.7e-12). We found that wrong methods transferred far better than simple arithmetic errors, which were nearly always caught. However, in another pass, detailed below, we found that this was due to the style of the drafts rather than their content; models followed clear, rewritten flawed drafts more often than raw CoT extracts.


| Flaw type (GPT-5.6 Luna) | Net error inheritance |
|---|---:|
| Wrong method | +49% |
| Right method, wrong execution | −12% |
| Arithmetic error or similar | −18% |
[note: tables and figures in this doc are AI-generated]

In another experiment, we assembled 80 coding and 34 math problems from USACO 2026, Codeforces 2026, LiveCodeBench v6 Hard, and MathArena hard and harvested 279 model errors from 77 problems. We measured what we termed the "net error inheritance," meaning the increase in _failing_ the same targeted test cases as the source solution. 
...
| Draft arm (GPT-5.6 Luna, 12 items, 10 rollouts each) | Net error inheritance | Solve rate (unaided: 66%) |
|---|---:|---:|
| Natural, raw failing submission | 17% | 58% |
| Natural, same flaw rewritten cleanly | 34% | 41% |
| Planted, newly authored | 6% | 68% |
| Correct draft | n/a | 83% |

We reran 12 medium difficulty items on GPT-5.6 Luna. Clean rewrites doubled inheritance (p=0.012). From scratch flawed traces generated by GPT-5.6 Sol were inherited less often. So the inheritance effects appear to be moderated by both the type of error and the style and cleanliness of the writeup. Correct drafts increased the solve rate, as expected.

We found some model-specific differences in inheriting hard problem vs easy problem errors. GPT-5.6 Sol only displayed +4.2% inheritance on problems with a >90% base solve rate, suggesting it was less misled by incorrect traces on these (comparison to non-easy problems stat sig, Mann-Whitney U p=7e-4). Luna, on the other hand, still had 35% inheritance on problems easy to it. In general, wrong drafts reduced solve rate by 30% for moderate difficulty problems, although in a few cases incorrect drafts helped models by providing an approach. Models also differed on whether they tended to inherit their own or other models' errors, with GPT-OSS-120B shipping its own errors more than other models', but Minimax m2.5 the reverse (+0.27 vs -0.33, both p < 1e-4). (This may simply be driven by the quality of the writeup, where we found that clean rewrites of solutions were more accepted than raw ones; in other words if model A writes cleaner solutions than B, both B and A may inherit A's solutions more, and this isn't a matter of "inheriting one's own errors more.")
...
_We think that these results suggest that solving problems and checking solutions are somewhat different capabilites_, and models can vary along these correlated dimensions. For example, on one item GPT-5.6 Sol had an 89% solve rate unaided and 0/20 when handed a failure. In another instance, GPT-OSS-120B detected an error 25/25 times but still submitted the error 19/25 times. 

## Limitations and Next Steps

(See the above intro and context for some bigger picture ideas regarding measuring other facets.)

1. Correct draft deference: we briefly measured deference to correct drafts; Luna used correct drafts, boosting its solve rate from 66% to 83%.
...
6. Agent swarms and cooperation: while this benchmark gestures toward it, we'd like to measure different framings such as a note being from a model's peer in a multiagent setup to check model deference. This would be relevant to HuggingFace Incident style setups where misaligned memes spread through a population. Excessive deference could be a contributing risk factor, allowing misaligned goals to spread through a swarm.
...
8. Prefill: We would like to compare how models inherit responses using API prefill (meaning it appears as the model's own thoughts) compared to simply handing them a partial response.

## (Appendix) Other Aspects of Jaggedness
...
3. Executive function and planning: without a manual planning step, models often jump in head first and start writing code haphazardly. This leads to confused, chaotic, spaghetti codebases and especially increased technical debt, which is hard to train against given the constraints of RL rollouts. (Long rollouts are very expensive but arguably required to teach long-term planning.) Likewise in research, they often start firing off small, incremental experiments without a working model of how this might lead to a novel or interesting result.
4. Research taste: while models execute reasonably well against simple numeric objectives, even in these cases they often use granular hyperparameter tweaks rather than larger conceptual changes. (For example, a human ML researcher might change the data filtering or sampling, add features, change the model architecture, or change the objective function or model specification rather than tweaking hyperparameter; as another example, a model also might run an experiment, get a weird/unexpected/glitchy result and not realize this should be debugged, instead coming up with a post hoc reason to accept the result and move on.) Again, this relates to myopia, and I speculate that the limited horizon of research RLVR environments causes models to be excellent at short term optimizations (HPO, kernel optimization) but not great at human weeks to especially human months long research projects that require conceptual cleverness and ingenuity.
5. Less verifiable domains: Given the difficulty of measuring these, they won't likely join this benchmark soon. But my experience is that models' prose quality hasn't improved in the RLVR era (since O1, late 2024); in addition, what I refer to as "RL smells" that perhaps result from RLVR have entered writing. Some examples of these include animist prose, where models animate inanimate subjects, negative ontologies, where models create fake insights by constantly describing things in terms of what they aren't, and hedge-y and overly verbose prose, where models are unable to control their logorrhea and write what someone actually wants to read, since their graders' rubrics rewarded completeness and comprehensiveness. It's easy to reward writing that is "creative," nonsensically metaphorical, purple, detailed, and fake-nuance laden. One could study the effects of LLM judging on prose, although translating this to a benchmark would be harder. Other less verifiable domains include business planning and, thankfully, military and economic coordination, along with artistic domains. 
